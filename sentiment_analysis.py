...
def plot_overall_sent(data_frame,file_writer):
    ...
    with open('plot_data_1.csv', 'w', newline='') as file:
        fieldnames = ['created_at', 'sentiment']
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writeheader()
        for index in data_frame.index:
            #writing the individual tweet sentiments and the timestamp of the tweet into the file
            writer.writerow({"created_at": data_frame["Actual Tweet"][index]["created_at"],"sentiment": data_frame["sentiment_polarity"][index]})
